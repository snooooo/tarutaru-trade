-- TaruTaru Trade: remove administrator-hidden posts from the public posts view.
-- Keeps closed trade posts as closed in trade_posts, while hiding them publicly
-- when admin_hidden_at is set.

begin;

create or replace view public.trade_public_posts as
select
  p.id,
  p.status,
  p.closed_at,
...
join lateral (
  ...
  where oi.trade_post_id = p.id
    and oi.status in ('public', 'closed')
...
left join lateral (
  ...
  where wi.trade_post_id = p.id
    and wi.status in ('public', 'closed')
) want_items on true
